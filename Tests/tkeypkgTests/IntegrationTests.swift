...
final class integrationTests: XCTestCase {
    ...
    func test_TssModule() async throws {
        ...
        // 2/2 -> 2/3 tss
        try await tss1.add_factor_pub(factor_key: factorKey.hex, auth_signatures: signatures, new_factor_pub: newFactorPub, new_tss_index: 3)

        // 2/3 -> 2/2 tss
        try await tss1.delete_factor_pub(factor_key: factorKey.hex, auth_signatures: signatures, delete_factor_pub: newFactorPub)
    ...
    func test_TssModule_multi_tag () async throws {
        ...
        for tag in testTags {
            ...
            try tssMod.create_tagged_tss_share(deviceTssShare: nil, factorPub: factorPub, deviceTssIndex: 2)
            
            
            let ( tssIndex, tssShare) = try! tssMod.get_tss_share(factorKey: factorKey.hex)
        ...
        // add factor key
        for (index, tssMod) in tssMods.enumerated() {
            ...
            try await tssMod.add_factor_pub(factor_key: factorKeys[index].hex, auth_signatures: signatures, new_factor_pub: newFactorPub, new_tss_index: 3)
            
            try await threshold.sync_local_metadata_transistions()
            
            let (tssIndex, tssShare) = try tssMod.get_tss_share(factorKey: factorKeys[index].hex)
            XCTAssertEqual(tssIndex, tssIndexes[index])
            XCTAssertNotEqual(tssShare, tssShares[index])

            let (tssIndex1, tssShare1) = try tssMod.get_tss_share(factorKey: newFactorKey.hex)
            XCTAssertNotEqual(tssIndex1, tssIndexes[index])
            XCTAssertNotEqual(tssShare1, tssShares[index])
            
            let newFactorKeyRandom = try PrivateKey.generate();
            let newFactorPubRandom = try newFactorKeyRandom.toPublic();
//
//            do {
//                try await tssMod.add_factor_pub(factor_key: factorKeys[index].hex, auth_signatures: signatures, new_factor_pub: newFactorPubRandom, new_tss_index: 3)
//                XCTFail("should not able to add factor with exising indexes")
//            }catch {
//
//                XCTFail("should not able to add factor with exising indexes")
//            }
        ...
        for (index, tssMod) in tssMods.enumerated() {
            ...
            try tssMod.copy_factor_pub(factorKey: newFactorKeys[index].hex, newFactorPub: newFactorPub2, tss_index: 3)
            
            let (tssIndex, tssShare) = try tssMod.get_tss_share(factorKey: factorKeys[index].hex)
            XCTAssertEqual(tssIndex, tssIndexes[index])
            XCTAssertNotEqual(tssShare, tssShares[index])

            let (tssIndex1, tssShare1) = try tssMod.get_tss_share(factorKey: newFactorKeys[index].hex)
            XCTAssertNotEqual(tssIndex1, tssIndexes[index])
            XCTAssertNotEqual(tssShare1, tssShares[index])
            
            let (tssIndex2, tssShare2) = try tssMod.get_tss_share(factorKey: newFactorKeys2[index].hex)
        ...
        var tssModsInstance2 : [TssModule] = []
        ...
        for (index, tssMod) in tssModsInstance2.enumerated() {
            ...
            newFactorKeys2.append(newFactorKey2)
            newFactorPubs2.append(newFactorPub2)
            try await tssMod.delete_factor_pub(factor_key: newFactorKeys[index].hex, auth_signatures: signatures, delete_factor_pub: newFactorPubs[index])
            
            XCTAssertThrowsError( try tssMod.get_tss_share(factorKey: newFactorKeys[index].hex) )
