...
final class tkey_pkgPrivateKeyModuleTests: XCTestCase {
    ...
    func test() async {
        ...
        _ = try! await PrivateKeysModule.set_private_key(thresholdKey: threshold_key, key: key1.hex, format: "secp256k1n")
        XCTAssertEqual(try! PrivateKeysModule.get_private_key_accounts(threshold_key: threshold_key), [key1.hex])
        _ = try! await PrivateKeysModule.set_private_key(thresholdKey: threshold_key, key: key2.hex, format: "secp256k1n")
        ...
        _ = try! await PrivateKeysModule.set_private_key(thresholdKey: threshold_key, key: nil, format: "secp256k1n")
